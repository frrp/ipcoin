...
const QString BITCOIN_IPC_PREFIX("ipcoin:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/ipcoin-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/ipcoin-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/ipcoin-payment";
...
bool PaymentServer::ipcSendCommandLine(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ipcoin: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click ipcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start ipcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling ipcoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on ipcoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in ipcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ipcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        if (GUIUtil::parseBitcoinURI(s, &recipient))
            ...
            emit message(tr("URI handling"),
                tr("URI can not be parsed! This can be caused by an invalid IPCoin address or malformed URI parameters."),
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom ipcoin addresses are not supported
