...
{
    return QCoreApplication::translate("ipcoin-core", psz).toStdString();
}
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. IPCoin can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. ipcoin_de.qm (shortcut "de" needs to be defined in ipcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. ipcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ipcoin.qrc)
...
int main(int argc, char *argv[])
{
    ...
    QApplication::setOrganizationName("IPCoin");
    QApplication::setOrganizationDomain("mechanicalamoeba.com");
    if (isaTestNet) // Separate UI settings for testnets
        QApplication::setApplicationName("IPCoin-Qt-testnet");
    else
        QApplication::setApplicationName("IPCoin-Qt");
    ...
    if (fMissingDatadir) {
        QMessageBox::critical(0, QObject::tr("IPCoin"),
                              QObject::tr("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(mapArgs["-datadir"])));
        return 1;
    }
    else if (fSelParFromCLFailed) {
        QMessageBox::critical(0, QObject::tr("IPCoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }

    // Start up the payment server early, too, so impatient users that click on
    // ipcoin: links repeatedly have their payment requests routed to this process:
        ...
        {
            {
                ...
                // ipcoin: URIs or payment requests:
